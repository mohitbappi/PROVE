<?php
    ...
    if (isset($_POST['mem']))
    {
        ...
        $stmt->execute(array(':dn' => $_REQUEST['mid'],':s'=>2));
		$date = date('Y-m-d');
		$stmt1 = $pdo->prepare('insert into Score (Member_id,Company_id,Points,Time) values ( :mid, :cid,:point,:time)');
        $stmt1->execute(array(':mid' => $_REQUEST['mid1'],':cid'=>$_SESSION['cid'],':point'=>$_REQUEST['point'],':time'=>$date));
...
<body>
    ...
    <form method="POST" action="mark.php" class="col-xs-5">

    <input type="submit" value="Confirm" name="mem" class="btn btn-info">
    <input type="hidden" name="mid" value="<?= $_GET['mid'] ?>">
	<input type="hidden" name="point" value="<?= $_GET['point'] ?>">
	<input type="hidden" name="mid1" value="<?= $_GET['mid1'] ?>">
